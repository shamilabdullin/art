...
class PaintingsApi {
  ...
  getPaintings(page: string): Promise<ResponsiveData> {
    return fetch(this.url + `search?query[term][is_public_domain]=true&page=${page}`).then(
  ...
  getPaintingsQuery(query: string = '', page: string = '1'): Promise<ResponsiveData> {
    ...
    return fetch(
      this.url + `search?${queryString}query[term][is_public_domain]=true&page=${page}`,
    ).then((response) => response.json())
  }

  getArtistsPaintings(query: string, artist_id: string, page: string): Promise<ResponsiveData> {
    return fetch(
      this.url + `search?q=${query}&query[term][artist_id]=${artist_id}&page=${page}`,
