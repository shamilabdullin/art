...
class ArtistsApi {
  ...
  getArtists(page: string): Promise<ResponsiveData> {
    return fetch(`https://api.artic.edu/api/v1/artists?page=${page}&limit=10`).then((response) =>
      response.json(),
    )
  }

  getArtist(id: string): Promise<ResponsiveData> {
    return fetch(`https://api.artic.edu/api/v1/artists/${id}`).then((response) => response.json())
  }

  getArtistsQuery(query: string = '', page: string): Promise<ResponsiveData> {
    return fetch(this.url + `search?q=${query}&page=${page}`).then((response) => response.json())
  }
}
