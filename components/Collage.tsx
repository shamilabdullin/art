			...
			{paintings ? 
				<div className={styles.collage}>
					{paintings.map((painting) => (
						...
						<Link href={`/paintings/${painting.id}`} key={painting.id} className={styles.painting} onClick={() => setQuery('')}>
							<Image src={noImg} alt={painting.title} className={styles.no_img}/>
							<div>
								<p className={styles.painting_title}><strong>{painting.title}</strong></p> {/* {painting.date_end > 0 ? painting.date_end : `${painting.date_end + painting.date_end * 2} B.C.`} */}
